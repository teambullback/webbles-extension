...
function add_Document(){
	...
	var pageCreator = createPageAsHtml(pagecount, next_width);
	next_width += 130;
...
function createPageAsHtml(pagecount, page_width) {
	return [ 
		'<div id="impagebar' + pagecount +'"  style="float:left" >',
		'<div  id="impagebar_up' + pagecount + '" style="width: ' + page_width +'; height :20px;" align="center">Page' + pagecount + '</div>',
...
// 서버에 페이지 정보가 저장되었을 때 호출되는 callback 함수
function success_on_save() {
	is_save = true;
}

function on_load(tutorial_num) {
...
function on_save(){ //저장 클릭시 (하나하나저장) 실제 
	...
	if(is_first_bubble){
		post_new_page('test', 'test', document.location.href, true,tutorial_num, success_on_save); //bubble에 뭐 넣을건지 생각 !
		is_first_bubble = false;
	}
	else{
		if(is_clicked){
			post_new_page('test', 'test', document.location.href, false,tutorial_num, success_on_save); //bubble에 뭐 넣을건지 생각 !
			is_clicked=false;
		}
		else
		{
		 	if(is_nextclick){ 
		 		console.log('next');
		 		post_new_bubble('test', 'test',"DIV","N",false,bubble_num,page_num, success_on_save);//dompath는 원경이에게 받은 값/  document는 post_new_page의 리턴값 id
		 	}
		 	else{
		 		console.log('click');
		 		post_new_bubble('title', 'description',"DIV","C",false,bubble_num,page_num, success_on_save);//dompath는 원경이에게 받은 값/  document는 post_new_page의 리턴값 id
			}
		}
	}	
};
