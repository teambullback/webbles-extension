...
function usermode(){
	...
	//사용자모드 나타낸다 
	   ...
	   for(var i =1 ; i<4; i++){ //모든값들 
		   //저장한 값들을 불러와 넣어준다. for
	    	var bubbleCreator_user = [
		    	'<div id="allbubble_user' + i + '" style="float:left;">',
		    		'<div style="width:150px; height:15%; "></div>',
		    		'<div id="bigbubble_user" style="width:150px; height:70%; ">',
		    			'<div style="width:100%; height:30%">',
			    			'<div id="eventbtn_user" style="float:left;width:40%; height:100%; background-color:green; background-size: 60px 40px; background-image: url(' + 'next.png' + ');"> </div>', //event버
			    			'<div style="float:left;width:20%; height:100%;"> </div>',
			    			'<div id="cnt_user" style="float:left;width:40%; height:100%;" align="center">#1</div>', //몇번째 버블인지 
		    			'</div>',
		    			'<div id="content_user' + i + '" style = "width:100%; height: 70%; background-color:blue;" align="center" > blahblah</div>',//내용 
		    		'</div>',
		    		'<div style="width:150px; height:15%;"></div>',
		    	'</div>',
		    	
		    	'<div id="allow' + i + '" style="float:left; width :50px; height: 100%; background-size: 60px 170px; background-image: url(' + 'arrow1.png' + ');"></div>'
		    ].join('\n');
		    $(bubbleCreator_user).appendTo('#myStatus_user');
		}
		    /*
		    //NEXT / CLICK인지 확인 후 넣기 
		    $('#eventbtn_user').css('background-image','none');
		    
		    $('#cnt_user').css('display','none');
		    $('#content_user').css('display','none');
		    */
		   
		   //첫번쨰로 포커스 맞추고 보여주기 
		    $('#content_user1').css('background-color','red');
